<template>
	<view class="content">
			...
			<view>
				...
				<view class="top_li">
					<view class="top_li_left">
						<image @click="back()" src="../../static/images/fanhui.png" mode="widthFix" class="fanghui">
						</image>
						<view class="jindu_box">
							<image src="../../static/images/new_yue_icon2.png" mode="widthFix" class="jidu_icon">
							</image>
							<text class="jindu">进度：{{jindu}}</text>
						</view>
						<view class="jindu_box">
							<image src="../../static/images/new_yue_icon3.png" mode="widthFix" class="jidu_icon">
							</image>
					...
					<view class="top_li_right">
						...
						<view class="yeBox">
							<view class="ye_li" @click="rotateClick" style="display: none;">
								<image src="../../static/images/new_yue_icon6.png" class="top_li_icon"></image>
								<text class="ye_li_text">翻转</text>
							</view>

							<view class="ye_li" @click="yinanClick" style="display: none;">
		...
		<div class="main">
			...
			<view class="dafenban_box">
				<scroll-view scroll-y="true" class="stscroll">
					<view class="shiti_li" v-for="(item,index) in pageQuickScores" :key="index"
						@click="quickScoreItemClick(index)"
						:class="questionScore[questionIndex]==item?'shiti_li_cur':''">
			...
			<view class="shiti">
				<scroll-view scroll-y="true" class="stscroll">
					<view class="shiti_itme" :class="questionIndex==index?'shiti_itme_cur':''"
						v-for="(item,index) in questions" @click="questionClick(index)">
			...
			<movable-area scale-area class="movable-area"
				...
				<movable-view class="movable-view" :direction="movableDir" scale="true" scale-min="1" scale-max="4"
					...
					<view class="picContent" v-for="(item,index) in papers" :key="index">
						<view class="xuanzhuanPic" @click="xztp(index)">
							<image src="../../static/images/xuanzhuan90.png" mode="widthFix" class="xz_img"></image>
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			xztp(index1) {
				let ds = parseInt(this.dglist[index1]) + 90;
				if (ds > 360) ds = ds - 360;
				this.dglist[index1] = ds;
				yu.dpost(2011, (result) => {
					if (result.state == 1) {
						this.papers.length = 0;
						this.pagePaperWidth.length = 0;
						this.pagePaperServerSize.length = 0;
 ...
 */
	...
	.yeBox {
		...
		line-height: 43rpx;
		margin-right: 5rpx;
		border: 1px solid rgb(115, 220, 208);
		border-radius: 10rpx;
		display: flex;
	}
	...
	.ye_li::after {
		...
		width: 1px;
		height: 30px;
		background-color: rgb(115, 220, 208);
	}

	.ye_li:last-child::after {
		width: 0;
	}
	...
	.shiti_itme_cur {
		border: #ff6527 solid 1px;
		background-color: #ff6527;
	}

	.shiti_itme_cur .ti_val {
		background-color: #ff6527;
		color: #fff;
	}

	.shiti_itme_cur .ti_name {
		background-color: #fff;
